...
describe("saving the settings a board keeps", () => {
  ...
  const harness = (appState: MutableRefObject<any>, orderSig = "same-order") => {
  ...
  it("writes a settings change that touches no element", () => {
    ...
    expect(debouncedSave.mock.calls[1][2]).toEqual({ objectsSnapModeEnabled: false });
  });
  ...
  it("does not write again while the settings stand still", () => {
    ...
    // Once, for the first change of the session. Panning and clicking about
    // must not keep writing the same settings back.
    expect(debouncedSave).toHaveBeenCalledTimes(1);
  });
});
