...
export const sanitizeSvg = (svgContent: string): string => {
  ...
  const sanitizeSvgImageTags = (content: string): string =>
    content.replace(/<image\b[^>]*>/gi, (imageTag) => {
      ...
      const withoutXlinkHref = imageTag.replace(/\sxlink:href\s*=\s*(?:"[^"]*"|'[^']*')/gi, "");
...
export const sanitizeText = (input: unknown, maxLength: number = 1000): string => {
  if (typeof input !== "string") return "";
...
export const appStateSchema = z
  .object({
    ...
    currentItemFillStyle: z.enum(["solid", "hachure", "cross-hatch", "dots"]).optional().nullable(),
    currentItemStrokeWidth: z.number().finite().min(0).max(50).optional().nullable(),
    currentItemStrokeStyle: z.enum(["solid", "dashed", "dotted"]).optional().nullable(),
    currentItemRoundness: z
      .union([
        z.enum(["sharp", "round"]),
        z.object({ type: z.enum(["round", "sharp"]), value: z.number().finite().min(0).max(1) }),
      ])
      .optional()
      .nullable(),
    currentItemFontSize: z.number().finite().min(1).max(500).optional().nullable(),
    currentItemFontFamily: z.number().finite().min(1).max(10).optional().nullable(),
    currentItemTextAlign: z.enum(["left", "center", "right"]).optional().nullable(),
    currentItemVerticalAlign: z.enum(["top", "middle", "bottom"]).optional().nullable(),
    scrollX: z.number().finite().min(-10000000).max(10000000).optional().nullable(),
    scrollY: z.number().finite().min(-10000000).max(10000000).optional().nullable(),
    zoom: z
    ...
    activeEmbeddable: z.object({ elementId: z.string(), state: z.string() }).optional().nullable(),
    activeTool: z
...
export const validateCsrfToken = (clientId: string, token: string): boolean => {
  ...
  try {
    ...
    const expectedSig = signCsrfToken(clientId, { ts: payload.ts, nonce: payload.nonce });
