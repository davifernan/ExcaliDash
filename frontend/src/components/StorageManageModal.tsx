...
function StatusIcon({ active, present }: { active?: boolean; present: boolean }) {
  if (active) {
    return (
      <span className="text-emerald-600 dark:text-emerald-400 font-bold" title="Active">
        {"\u2713"}
      </span>
    );
  }
  if (present) {
    return (
      <span className="text-amber-500 dark:text-amber-400 font-bold" title="History-only">
        {"\u25D0"}
      </span>
    );
  }
  return (
    <span className="text-neutral-400 dark:text-neutral-500 font-bold" title="Missing">
      {"\u2717"}
    </span>
  );
}
      ...
      <div className="relative w-full max-w-3xl max-h-[90vh] flex flex-col bg-white dark:bg-neutral-900 rounded-2xl border-2 border-black dark:border-neutral-700 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_0px_rgba(255,255,255,0.08)] animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center gap-3 p-5 pb-4 border-b-2 border-black dark:border-neutral-700">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-neutral-500 dark:text-neutral-400 truncate">{drawingName}</p>
          </div>
          <button
            onClick={onClose}
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-neutral-100 dark:bg-neutral-800 border-b-2 border-black dark:border-neutral-700">
                          ...
                          <th className="px-3 py-2 text-left font-bold text-neutral-700 dark:text-neutral-300">
                            File ID
                          </th>
                          <th className="px-3 py-2 text-center font-bold text-neutral-700 dark:text-neutral-300">
                            Canvas
                          </th>
                          <th className="px-3 py-2 text-center font-bold text-neutral-700 dark:text-neutral-300">
                            SQLite
                          ...
                          <th className="px-3 py-2 text-right font-bold text-neutral-700 dark:text-neutral-300">
                            Size
                          </th>
                        </tr>
                      </thead>
                        ...
                        {diffData.files.length === 0 ? (
                          ...
                          diffData.files.map((file: FileDiffEntry) => (
                            ...
                            >
                              ...
                              <td
                                className="px-3 py-2 text-neutral-900 dark:text-neutral-100 font-mono text-xs truncate max-w-[200px]"
                                title={file.fileId}
                              >
                                {file.fileId}
                              </td>
