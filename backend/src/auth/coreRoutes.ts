...
import { issueBootstrapSetupCodeIfRequired, verifyBootstrapSetupCode } from "./bootstrapSetupCode";
...
type RegisterCoreRoutesDeps = {
  ...
  config: {
    ...
    oidc: { enabled: boolean; enforced: boolean; providerName: string; jitProvisioning: boolean };
  ...
  getRefreshTokenExpiresAt: (remember?: boolean) => Date;
...
export const registerCoreRoutes = (deps: RegisterCoreRoutesDeps) => {
    ...
    try {
      ...
      if (isBootstrapFlow) {
        ...
        if (!setupCodeVerification.ok) {
          ...
          if (reason === "unavailable" || reason === "expired" || reason === "locked") {
          ...
          if (reason === "invalid") {
            return res
              .status(401)
              .json({ error: "Unauthorized", message: "Invalid bootstrap setup code" });
          }
          if (reason === "locked") {
        ...
        if (!existingTrash) {
          await prisma.collection.create({
            data: { id: trashCollectionId, name: "Trash", userId: user.id },
          });
        }
      ...
      if (!systemConfig.registrationEnabled) {
        return res
          .status(403)
          .json({ error: "Forbidden", message: "User registration is disabled." });
      }
      ...
      if (username) {
        ...
        if (existingUsername) {
          return res
            .status(409)
            .json({ error: "Conflict", message: "User with this username already exists" });
        }
      }
      const saltRounds = 10;
      const passwordHash = await bcrypt.hash(password, saltRounds);
      ...
      const user = await prisma.user.create({
        data: { email, passwordHash, name: sanitizedName, username: username ?? null },
        select: {
          id: true,
          email: true,
          name: true,
  ...
  router.post("/login", loginAttemptRateLimiter, async (req: Request, res: Response) => {
    try {
      ...
      if (config.authMode === "oidc_enforced") {
        return res
          .status(403)
          .json({ error: "Forbidden", message: "Local login is disabled in OIDC enforced mode" });
      }
      if (!requireCsrf(req, res)) return;
      ...
      if (!parsed.success) {
        return res
          .status(400)
          .json({ error: "Validation error", message: "Invalid login credentials" });
      }
      ...
      const { accessToken, refreshToken } = generateTokens(user.id, user.email, { remember });
      setAuthCookies(req, res, { accessToken, refreshToken }, { remember });
      if (config.enableRefreshTokenRotation) {
        const expiresAt = getRefreshTokenExpiresAt(remember);
        try {
          await prisma.refreshToken.create({
            data: { userId: user.id, token: hashTokenForStorage(refreshToken), expiresAt },
          });
          ...
          try {
            ...
            await prisma.$transaction(async (tx) => {
              ...
              await tx.refreshToken.create({
                data: { userId: user.id, token: hashTokenForStorage(newRefreshToken), expiresAt },
              });
            });
            setAuthCookies(req, res, { accessToken, refreshToken: newRefreshToken }, { remember });
  ...
  router.post("/stop-impersonation", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (!req.user.impersonatorId) {
        return res
          .status(409)
          .json({ error: "Conflict", message: "Not currently impersonating another user" });
      }
      ...
      const { accessToken, refreshToken } = generateTokens(impersonator.id, impersonator.email);
      setAuthCookies(req, res, { accessToken, refreshToken });
      if (config.enableRefreshTokenRotation) {
        const expiresAt = getRefreshTokenExpiresAt();
        try {
          await prisma.refreshToken.create({
            data: { userId: impersonator.id, token: hashTokenForStorage(refreshToken), expiresAt },
          });
        } catch (error) {
          if (isMissingRefreshTokenTableError(error)) {
            return res.status(503).json({
  ...
  router.get("/status", optionalAuth, async (req: Request, res: Response) => {
    try {
      ...
      const effectiveAuthEnabled = config.authMode !== "local" ? true : systemConfig.authEnabled;
      ...
      res.json(
        buildAuthStatusPayload({
          ...
          passwordResetEnabled: Boolean(config.enablePasswordReset && mailer?.enabled),
          user: req.user,
        }),
      );
    } catch (error) {
  ...
  router.post("/onboarding-choice", optionalAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (!parsed.success) {
        return res
          .status(400)
          .json({ error: "Bad request", message: "Invalid onboarding choice payload" });
      }
      const systemConfig = await ensureSystemConfig();
      const onboarding = await getAuthOnboardingStatus(prisma, systemConfig);
